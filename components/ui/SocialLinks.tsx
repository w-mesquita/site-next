...
const ICONS: Record<
  SocialNetworkKey,
  { label: string; color: string; path: React.ReactNode }
> = {
  instagram: {
    label: "Instagram",
    color: "#E4405F",
...
export function SocialLinks({ size = "md", className = "" }: SocialLinksProps) {
  ...
  const items = (["instagram", "facebook", "linkedin"] as const).filter(
    (key) => config[key].enabled && config[key].url.trim() !== ""
  );
      ...
      {items.map((key) => {
        ...
        return (
          <Link
            ...
            aria-label={label}
            className="rounded-full p-1.5 transition-opacity hover:opacity-80 focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:ring-offset-2 focus:ring-offset-transparent"
            style={{ color }}
          >
            <svg
              className={iconSize}
